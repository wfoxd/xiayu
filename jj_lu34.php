...
$way = array("dl/jj_lu34.php","dl/jj_lu26.php");
area_now($way,$user_id);
//记录位置
include "../include/location_lu.inc.php";
up_location("荆棘路","dl/jj_lu34.php","$user_id");
?>
<?
	echo "<br>=============荆棘路===============<br>";
	...
	$bao_time = mysql_query("SELECT bao2 FROM dl_bao");
	$bao_time = mysql_result($bao_time,0,"bao2");
	$now_time = time();
	$my_info = mysql_query("SELECT yun FROM renwu_member WHERE id='$user_id'");
	$my_yun = mysql_result($my_info,0,"yun");
	$add_exp = $my_yun * 20 + 400;
	$add_mon = $my_yun * 10 + 100;
	...
	if(($now_time - $bao_time) > 3600){
		...
		mysql_query("UPDATE dl_bao SET bao2='$now_time'");
		mysql_close();		
	}else{
