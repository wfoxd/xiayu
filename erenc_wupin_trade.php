...
$way = array("ereng/erenc.php","ereng/erenc_wupin_trade.php");
area_now($way,$user_id);

include "../include/location_lu.inc.php";
up_location("无亲装备店","ereng/erenc_wupin_trade.php","$user_id");
?>
...
if($buy == 1){
	...
	echo "<br><a href=erenc_wupin_trade.php?begin_buy=1><font color=green>购买</font></a>\n";
...
<?
	...
	echo "<br>==============无亲装备店==============<br>";
	echo ("
		恶人村里有不少特别的工匠打造了不少很特殊的物品，【无亲装备店】就是专门卖这些东西的装备店，这里的老板无亲是个很孤僻的人，一般人最好不要惹他。
	");
    	$npc_org = "无亲装备店";
	...
	if($wupin_num > 0){
	echo "<form action=erenc_wupin_trade.php method=post>\n";
    ...
    for($k=0;$k<count($cla);$k++){
    	...
    	$gongfu = mysql_query("SELECT id,name FROM zhuangbei_wupin WHERE cla='$cla[$k]' AND hidden='N' AND here='恶人村' ORDER BY pri ASC");
        ...
        for($i=0;$i<=$gongfu_num-1;$i++){
        	...
        	echo "<option value=erenc_wupin_trade.php?buy=1&D1=$gongfu_id>".$gongfu_name."</option>\n";
        }
