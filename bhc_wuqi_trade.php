...
$way = array("bhc/bhc_wuqi_trade.php","bhc/bhc.php");
area_now($way,$user_id);

include "../include/location_lu.inc.php";
up_location("百花武器店","bhc/bhc_wuqi_trade.php","$user_id");
?>
<?
if($begin_buy == 1){	
	...
	echo "<meta http-equiv=\"refresh\" content=\"2; url=bhc_wuqi_trade.php\">";
	mysql_close();
	exit();
}
?>
...
if($buy == 1){
	...
	echo "<br><a href=bhc_wuqi_trade.php?begin_buy=1><font color=green>购买</font></a>\n";
...
<?
	...
	echo "<br>==============百花武器店==============<br>";
	echo ("这是【百花城】里最大的武器商店，你在这里可以买到百花城特有的武器。
	");
	...
	if($wuqi_num){
	echo "<form action=bhc_wuqi_trade.php method=post>\n";
    ...
    for($k=0;$k<count($cla);$k++){
    	...
    	$gongfu = mysql_query("SELECT id,name FROM zhuangbei_wuqi WHERE cla='$cla[$k]' AND hidden='N' AND here='百花城' ORDER BY pri ASC");
        ...
        for($i=0;$i<=$gongfu_num-1;$i++){
        	...
        	echo "<option value=bhc_wuqi_trade.php?buy=1&D1=$gongfu_id>".$gongfu_name."</option>\n";
        }
